...
create table attendance( --출석테이블 
		...
		num number,
		state varchar2(10),
		foreign key(num) references member(num) on delete cascade -- foreign key 지정, member테이블 num참조
);
...
create table grade(        -- 성적테이블
		num number primary key,   -- 회원 번호로 누군지 식별함, 멤버테이블 num 참조
		exam1 number,			-- 시험성적
		exam2 number,
		exam3 number,			-- 영업사원이 수강신청 받아주면 레벨 1>>2로 올리고, 성적테이블에 이름과 넘버 insert
...
create table recruit (--모집공고게시판>>select * from lecture; >>> 모집공고 업로드 하는 form >> 입력 >> 리쿠르트테이블에 insert
...
-- 일반회원 lvl = 0
-- 수료생 -- lvl = 1
-- 수강신청버튼은 레벨이 0인 회원만 누를수 있다
-- 일반회원이 수강신청 버튼 눌렀을때 update member set lecture = '신청한 강좌명' where num <=1(수료생 or 일반회원)

-- 예전 날짜로 출석체크 업데이트 했을때 // 오늘날짜로 넘어감.
